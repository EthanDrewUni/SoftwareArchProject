...
public class CompetitorList {
    ...
    public String getShortDetailsByCompetitorNumber(int competitorNumber) {
        for (Sprinter sprinter : sprinters) {
            if (sprinter.getCompetitorNumber() == competitorNumber) {
                return sprinter.getShortDetails();
            }
        }
    ...
    public Sprinter getCompetitorDetailsByCompetitorNumber(int competitorNumber) {
        for (Sprinter sprinter : sprinters) {
            if (sprinter.getCompetitorNumber() == competitorNumber) {
                return sprinter;
            }
    ...
    // Function to get details individually by competitor number
    public String getDetailsByCompetitorNumber(int competitorNumber) {
        Sprinter sprinter = getCompetitorDetailsByCompetitorNumber(competitorNumber);
        if (sprinter != null) {
            return "Competitor Number: " + sprinter.getCompetitorNumber() +
                    "\nFirst Name: " + sprinter.getName().getFirstName() +
                    "\nLast Name: " + sprinter.getName().getSurname() +
                    "\nCountry: " + sprinter.getCountry() +
                    "\nGender: " + sprinter.getGender() +
                    "\nAge: " + sprinter.getAge() +
                    "\nLevel: " + sprinter.getLevel() +
                    "\nScores: " + Arrays.toString(sprinter.getScoreArray()) +
                    "\nOverall Score: " + sprinter.getOverallScore();
        }
        else {
            return "Competitor not found with number: " + competitorNumber;
        }
    ...
    public boolean updateSprinterDetails(int competitorNumber, Sprinter modifiedSprinter) {
        for (Sprinter sprinter : sprinters) {
            if (sprinter.getCompetitorNumber() == competitorNumber) {
                sprinter.setName(modifiedSprinter.getName());
                sprinter.setCountry(modifiedSprinter.getCountry());
                sprinter.setAge(modifiedSprinter.getAge());
                sprinter.setLevel(modifiedSprinter.getLevel());
                return true;
            }
        }
        return false;
    ...
    public void setSprinterScoresByCompNumber(int competitorNumber, int[] newScores) {
        for (Sprinter sprinter : sprinters) {
            if (sprinter.getCompetitorNumber() == competitorNumber) {
                sprinter.setScores(newScores);
                break;
            }
        }
    }
